...
| `tests/test_reactor0/test_decision_making/test_pace_clue_gate.cpp` | The two gate windows (DECISION_MAKING.md, *Decision phase 1*) — both token boundaries incl. `clue_tokens == 3`, the HIGH row reaching past 3, the 8-token exemption, the no-stamp negative that pins the split, and the pace boundary at `pace() >= 1` (silent at 0, firing at 1, and firing at 2 where the pre-v7.4.0 `>= 3` threshold stood down) — plus required tier, and that reactor's own gate is unchanged |
